"""
Unit tests for daily EOD data ingestion script.
"""
...
import pandas as pd
from scripts.ingest_daily import (
    USEquityFetcher,
    CNAshareFetcher,
    HKSGEquityFetcher,
...
from scripts import STANDARD_COLUMNS
...
class TestPartitionedParquetWriter:
    ...
    def test_write_to_partitioned_parquet(self, tmp_path, sample_ohlcv_data):
        ...
        with patch('scripts.ingest_daily.US_EQUITY_DIR', market_dir):
...
class TestPipelineIntegration:
    ...
    def test_run_daily_ingestion_dry_run(self, tmp_path, sample_ohlcv_data):
        ...
        # Mock fetchers
        with patch('scripts.ingest_daily.fetch_market_data') as mock_fetch:
            mock_fetch.return_value = sample_ohlcv_data

            # Mock directory constants
            with patch('scripts.ingest_daily.US_EQUITY_DIR', tmp_path / 'us_equity'):
                with patch('scripts.ingest_daily.CN_ASHARE_DIR', tmp_path / 'cn_ashare'):
                    with patch('scripts.ingest_daily.HK_SG_EQUITY_DIR', tmp_path / 'hk_sg_equity'):
