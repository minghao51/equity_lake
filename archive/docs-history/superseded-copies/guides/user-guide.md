...
> **📌 Note:** This guide covers the original daily ingestion system. For the **new automated ML pipeline** (ingestion → features → ML), see [Pipeline Usage Guide](../../../docs/user-guide/pipeline.md) and [Quick Start Guide](../../../docs/getting-started/quickstart.md).
...
0 8 * * 1-5 cd /path/to/equity-eod && /path/to/.venv/bin/uv run equity-daily --parallel >> logs/daily.log 2>&1
```

3. **Alternative with email notifications**:
```bash
# Daily at 8 AM with email on error
0 8 * * * cd /path/to/equity-eod && /path/to/.venv/bin/uv run equity-daily --parallel >> logs/daily.log 2>&1 || mail -s "Ingestion Failed" user@example.com < logs/daily.log
```
