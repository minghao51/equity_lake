...
This script handles the initial one-time sync of historical US equity data
...
Usage:
    python -m scripts.sync_from_s3
    python -m scripts.sync_from_s3 --bucket s3://my-bucket/us_equity/
    python -m scripts.sync_from_s3 --workers 32 --dry-run
"""
...
Examples:
  ...
  python -m scripts.sync_from_s3 --tool s5cmd --workers 32

  # Dry run (test without downloading)
  python -m scripts.sync_from_s3 --dry-run

  # Sync to custom directory
  python -m scripts.sync_from_s3 --target /path/to/data
        """
    )
