...
python -m scripts.ingest_daily -p
```
...
python -m scripts.ingest_daily --date 2024-12-01 --markets us,cn,hk_sg --parallel

# Parallel mode with filters
python -m scripts.ingest_daily --parallel --tags blue-chip --min-priority 8

# Dry run with parallel mode (test without writing)
python -m scripts.ingest_daily --parallel --dry-run --verbose
```
...
```
2024-12-01 08:00:00 - scripts.ingest_daily - INFO - Fetching US equity data for 2024-12-01
2024-12-01 08:00:05 - scripts.ingest_daily - INFO - Fetched 5000 rows for US equities
```

#### After (Structured JSON)

```json
{"timestamp": "2024-12-01T08:00:00.000Z", "logger": "scripts.ingest_daily", "level": "info", "event": "market_fetch_started", "market": "us", "trading_date": "2024-12-01", "correlation_id": "a1b2c3d4"}

{"timestamp": "2024-12-01T08:00:05.123Z", "logger": "scripts.ingest_daily", "level": "info", "event": "market_fetch_completed", "market": "us", "row_count": 5000, "duration_seconds": 5.123, "correlation_id": "a1b2c3d4"}
```
...
```python
from scripts.logging_utils import timed, timer
...
```python
from scripts.logging_utils import timer
...
```python
from scripts.logging_utils import correlation_context
...
python -m scripts.ingest_daily --parallel --verbose
```
