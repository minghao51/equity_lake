...
Your current `src/equity_lake/validators.py` is excellent for catching typos:
```python
# US: ^[A-Z]{1,5}(-[A-Z]{1,2})?$
# CN: ^\d{6}$
# HK: ^\d{4}\.HK$
...
```python
# src/equity_lake/ticker_validator.py
...
```python
# equity_lake/config.py enhancement

# Proposed addition to equity_lake.validators
from equity_lake.validators import TickerValidator
...
```python
# equity_lake/ingest_incremental.py
...
1. **Add Gap Detection**
   ```bash
   # 1. Create src/equity_lake/gap_detector.py (use code above)
