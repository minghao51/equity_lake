...
> **Note:** This guide covers the day-to-day ingestion workflow. For the automated ML pipeline, see [Pipeline Usage Guide](pipeline.md) and [Quick Start Guide](../getting-started/quickstart.md).
...
The `equity-daily` command fetches and appends EOD data for multiple markets.
...
uv run equity-daily  # (no --parallel flag)

# Reduce worker count
uv run equity-daily --parallel --max-workers 1

# Add delays between requests (adjust the ingestion source settings)
...
0 8 * * 1-5 cd /path/to/equity-eod && uv run equity-daily --parallel >> logs/daily.log 2>&1
```

3. **Alternative with email notifications**:
```bash
# Daily at 8 AM with email on error
0 8 * * * cd /path/to/equity-eod && uv run equity-daily --parallel >> logs/daily.log 2>&1 || mail -s "Ingestion Failed" user@example.com < logs/daily.log
```
...
0 0 * * * cd /path/to/equity-eod && uv run equity-daily

# Every weekday at 6:30 PM (after market close)
30 18 * * 1-5 cd /path/to/equity-eod && uv run equity-daily --parallel

# Every 6 hours
0 */6 * * * cd /path/to/equity-eod && uv run equity-daily

# At 2 AM on Sundays (for weekend data)
0 2 * * 0 cd /path/to/equity-eod && uv run equity-daily --date $(date -d "yesterday" +%Y-%m-%d)
```
...
cd /path/to/equity-eod && uv run equity-daily --dry-run
```
...
Usage:
```bash
uv run equity-daily --tags blue-chip --min-priority 9
```
...
uv run equity-daily --parallel --max-workers 20

# Fetch more CN stocks
uv run equity-daily --parallel --markets cn --stock-limit 500
```

**For more reliable fetching**:
```bash
# Conservative settings
uv run equity-daily --parallel --max-workers 3

# Fewer stocks, more retries
uv run equity-daily --stock-limit 50 --retry-attempts 5
```
...
docker compose exec daily uv run equity-daily --dry-run
